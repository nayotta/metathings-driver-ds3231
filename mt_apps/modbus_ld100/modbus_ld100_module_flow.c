...
#include "modbus_ld100_module_flow.h"
...
static const char *TAG = "MODULE_LD100_FLOW";

// global func ================================================================

static void module_ld100_flow_loop(mt_module_flow_t *module_flow) {
  ...
  while (true) {
    data = modbus_ld100_get_data(1);
    if (data == NULL) {
      ESP_LOGE(TAG, "%4d %s modbus_ld100_get_data failed", __LINE__, __func__);
...
void modbus_ld100_module_flow_task(mt_module_flow_t *module_flow) {
  xTaskCreate((TaskFunction_t)module_ld100_flow_loop, "MODULE_FLOW", 8 * 1024,
              module_flow, 10, NULL);
}
