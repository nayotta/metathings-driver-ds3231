...
esp_err_t ds3231_init_desc(i2c_dev_t *dev, i2c_port_t port, gpio_num_t sda_gpio,
                           gpio_num_t scl_gpio) {
  CHECK_ARG(dev);
...
esp_err_t ds3231_set_alarm(i2c_dev_t *dev, ds3231_alarm_t alarms,
  ...
  I2C_DEV_TAKE_MUTEX(dev);
  I2C_DEV_CHECK(
      dev, i2c_dev_write_reg(dev,
                             (alarms == DS3231_ALARM_2 ? DS3231_ADDR_ALARM2
                                                       : DS3231_ADDR_ALARM1),
                             data, i));
  I2C_DEV_GIVE_MUTEX(dev);
 ...
 */
static esp_err_t ds3231_get_flag(i2c_dev_t *dev, uint8_t addr, uint8_t mask,
                                 uint8_t *flag) {
  uint8_t data;
 ...
 * returns true to indicate success
 */
static esp_err_t ds3231_set_flag(i2c_dev_t *dev, uint8_t addr, uint8_t bits,
                                 uint8_t mode) {
  uint8_t data;
...
esp_err_t ds3231_get_oscillator_stop_flag(i2c_dev_t *dev, bool *flag) {
  ...
  I2C_DEV_TAKE_MUTEX(dev);
  I2C_DEV_CHECK(dev, ds3231_get_flag(dev, DS3231_ADDR_STATUS,
                                     DS3231_STAT_OSCILLATOR, &f));
  I2C_DEV_GIVE_MUTEX(dev);
...
esp_err_t ds3231_clear_oscillator_stop_flag(i2c_dev_t *dev) {
  ...
  I2C_DEV_TAKE_MUTEX(dev);
  I2C_DEV_CHECK(dev, ds3231_set_flag(dev, DS3231_ADDR_STATUS,
                                     DS3231_STAT_OSCILLATOR, DS3231_CLEAR));
  I2C_DEV_GIVE_MUTEX(dev);
...
esp_err_t ds3231_get_alarm_flags(i2c_dev_t *dev, ds3231_alarm_t *alarms) {
  ...
  I2C_DEV_TAKE_MUTEX(dev);
  I2C_DEV_CHECK(dev, ds3231_get_flag(dev, DS3231_ADDR_STATUS, DS3231_ALARM_BOTH,
                                     (uint8_t *)alarms));
  I2C_DEV_GIVE_MUTEX(dev);
...
esp_err_t ds3231_enable_alarm_ints(i2c_dev_t *dev, ds3231_alarm_t alarms) {
  ...
  I2C_DEV_TAKE_MUTEX(dev);
  I2C_DEV_CHECK(dev,
                ds3231_set_flag(dev, DS3231_ADDR_CONTROL,
...
esp_err_t ds3231_disable_alarm_ints(i2c_dev_t *dev, ds3231_alarm_t alarms) {
  ...
  I2C_DEV_TAKE_MUTEX(dev);
  I2C_DEV_CHECK(
      dev, ds3231_set_flag(dev, DS3231_ADDR_CONTROL, alarms, DS3231_CLEAR));
  I2C_DEV_GIVE_MUTEX(dev);
...
esp_err_t ds3231_enable_32khz(i2c_dev_t *dev) {
  ...
  I2C_DEV_TAKE_MUTEX(dev);
  I2C_DEV_CHECK(dev, ds3231_set_flag(dev, DS3231_ADDR_STATUS, DS3231_STAT_32KHZ,
                                     DS3231_SET));
  I2C_DEV_GIVE_MUTEX(dev);
...
esp_err_t ds3231_enable_squarewave(i2c_dev_t *dev) {
  ...
  I2C_DEV_TAKE_MUTEX(dev);
  I2C_DEV_CHECK(dev, ds3231_set_flag(dev, DS3231_ADDR_CONTROL,
                                     DS3231_CTRL_ALARM_INTS, DS3231_CLEAR));
  I2C_DEV_GIVE_MUTEX(dev);
...
esp_err_t ds3231_disable_squarewave(i2c_dev_t *dev) {
  ...
  I2C_DEV_TAKE_MUTEX(dev);
  I2C_DEV_CHECK(dev, ds3231_set_flag(dev, DS3231_ADDR_CONTROL,
                                     DS3231_CTRL_ALARM_INTS, DS3231_SET));
  I2C_DEV_GIVE_MUTEX(dev);

  return ESP_OK;
}

esp_err_t ds3231_set_squarewave_freq(i2c_dev_t *dev,
                                     ds3231_sqwave_freq_t freq) {
  CHECK_ARG(dev);
  ...
  flag |= freq;
  I2C_DEV_CHECK(
      dev, ds3231_set_flag(dev, DS3231_ADDR_CONTROL, flag, DS3231_REPLACE));
  I2C_DEV_GIVE_MUTEX(dev);
...
esp_err_t ds3231_get_raw_temp(i2c_dev_t *dev, int16_t *temp) {
  ...
  I2C_DEV_TAKE_MUTEX(dev);
  I2C_DEV_CHECK(dev,
                i2c_dev_read_reg(dev, DS3231_ADDR_TEMP, data, sizeof(data)));
  I2C_DEV_GIVE_MUTEX(dev);
