 ...
 * every second alarm2 works with minutes, hours and day of week/month, or fires
 * every minute not all combinations are supported, see DS3231_ALARM1_* and
 ...
 * same time (pass DS3231_ALARM_1/DS3231_ALARM_2/DS3231_ALARM_BOTH) if only
 * setting one alarm just pass 0 for tm struct and option field for the other
 * alarm if using DS3231_ALARM1_EVERY_SECOND/DS3231_ALARM2_EVERY_MIN you can
 * pass 0 for tm stuct if you want to enable interrupts for the alarms you need
...
/**
 * @brief Check if oscillator has previously stopped, e.g. no power/battery or
 * disabled sets flag to true if there has been a stop
...
/**
 * @brief Disable the squarewave output (which re-enables alarm interrupts, but
 * individual alarm interrupts also need to be enabled, if not already, before
 * they will trigger)
 * @param dev Device descriptor
 * @return ESP_OK to indicate success
 */
