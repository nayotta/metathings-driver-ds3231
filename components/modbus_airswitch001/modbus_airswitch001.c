...
// sync cmd 01
static eMBMasterReqErrCode modbus_airswitch001_sync_cmd_01(
...
// sync cmd 02
static eMBMasterReqErrCode modbus_airswitch001_sync_cmd_02(
    UCHAR slaveAddr, USHORT target, USHORT num, struct RetMsg_t *ret)
{
  eMBMasterReqErrCode errorCode = MB_MRE_NO_ERR;
...
// sync cmd 03
static eMBMasterReqErrCode modbus_airswitch001_sync_cmd_03(
    UCHAR slaveAddr, UCHAR subCmd, UCHAR target, USHORT num,
    struct RetMsg_t *ret)
{
  eMBMasterReqErrCode errorCode = MB_MRE_NO_ERR;
...
// sync cmd 04
static eMBMasterReqErrCode modbus_airswitch001_sync_cmd_04(
    UCHAR slaveAddr, UCHAR subCmd, UCHAR target, USHORT num,
    struct RetMsg_t *ret)
{
  eMBMasterReqErrCode errorCode = MB_MRE_NO_ERR;
                                    ...
                                    bool *state)
{
  ...
  // cmd process
  err = modbus_airswitch001_sync_cmd_01(slaveAddr, target, 1, &cmd_ret_payload);

  // ret check
  if (err != ESP_OK)
  {
    ESP_LOGE(TAG, "%4d %s addr:%d target:%d failed", __LINE__, __func__,
             slaveAddr, target);
    return err;
  }
  ...
  if (cmd_ret_payload.recvCmd != CMD_01_STAT_READ)
  {
    ESP_LOGE(TAG, "%4d %s addr:%d target:%d get error ret cmd:%d", __LINE__,
             __func__, slaveAddr, target, cmd_ret_payload.recvCmd);
    return ESP_ERR_INVALID_RESPONSE;
  }

  if (cmd_ret_payload.retLen != 1)
  {
    ESP_LOGE(TAG, "%4d %s addr:%d target:%d response process failed", __LINE__,
             __func__, slaveAddr, target);
    return false;
  }
  ...
  {
    ESP_LOGE(TAG, "%4d %s addr:%d target:%d response get error value:%d",
             __LINE__, __func__, slaveAddr, target, cmd_ret_payload.retBuf[0]);
    return ESP_ERR_INVALID_RESPONSE;
  }
...
esp_err_t mt_airswitch001_get_ctrl(UCHAR slaveAddr, USHORT target, bool *ctrl)
{
  ...
  // cmd process
  err = modbus_airswitch001_sync_cmd_02(slaveAddr, target, 1, &cmd_ret_payload);
                                   ...
                                   USHORT *value_out)
{
  ...
  case DATA_QUALITY_H:
    break;
  default:
    ESP_LOGE(TAG, "%4d %s addr:%d target:%d get unexcept type:%d", __LINE__,
             __func__, slaveAddr, target, type);
    return ESP_ERR_INVALID_ARG;
    break;
  }
  err = modbus_airswitch001_sync_cmd_03(slaveAddr, type, target, 1,
                                        &cmd_ret_payload);
  ...
  if (cmd_ret_payload.retLen != 2)
  {
    ESP_LOGE(TAG, "%4d %s addr:%d target:%d get error ret size:%d", __LINE__,
             __func__, slaveAddr, target, cmd_ret_payload.retLen);
    return ESP_ERR_INVALID_RESPONSE;
  }
                                     ...
                                     USHORT *value_out)
{
  ...
  err = modbus_airswitch001_sync_cmd_04(slaveAddr, type, target, 1,
                                        &cmd_ret_payload);
  ...
  {
    ESP_LOGE(TAG, "%4d %s addr:%d target:%d get error ret cmd:%d", __LINE__,
             __func__, target, slaveAddr, cmd_ret_payload.recvCmd);
    return ESP_ERR_INVALID_RESPONSE;
  }
                                    ...
                                    bool state)
{
  ...
  if (state == true)
  {
    err = modbus_airswitch001_sync_cmd_06(slaveAddr, CFG_WRITE_SET_STAT, target,
                                          0x5A, &cmd_ret_payload);
  }
  else
  {
    err = modbus_airswitch001_sync_cmd_06(slaveAddr, CFG_WRITE_SET_STAT, target,
                                          0xA5, &cmd_ret_payload);
  }
                                     ...
                                     USHORT target, USHORT value)
{
  ...
  err = modbus_airswitch001_sync_cmd_06(slaveAddr, type, target, value,
                                        &cmd_ret_payload);
