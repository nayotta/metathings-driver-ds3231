...
static const char *TAG = "MODBUS_AEW100_HUAWEI_MAIN";
...
void app_main() {
  ...
  // device init
  err = modbus_aew100_with_config_init(UART_PORT, TX_PIN, RX_PIN, EN_PIN);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "%4d %s modbus_aew100_with_config_init failed", __LINE__,
             __func__);
    return;
  }
  ...
  module_flow->flow_handle = modbus_aew100_get_flow_data;
