#include "stdint.h"

#include "esp_log.h"
#include "freertos/FreeRTOS.h"
#include "freertos/queue.h"
#include "freertos/task.h"

#include "modbus_ld100.h"

// global define ==============================================================

static const char *TAG = "modbus_ld100_base";

uint8_t RS485_PORT = 2;
int TX_PIN = 13;
int RX_PIN = 15;
int EN_PIN = 05;

int ADDR = 1;

// test func =================================================================

void test_ld100_get_addr() {
  esp_err_t err = ESP_OK;
  bool state = true;

  err = modbus_ld100_get_addr(ADDR, &state);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "%4d %s failed code:%d", __LINE__, __func__, err);
    return;
  }

  ESP_LOGI(TAG, "%4d %s ret:%d", __LINE__, __func__, state);

  return;
}

void test_ld100_set_addr() {
  esp_err_t err = ESP_OK;

  err = modbus_ld100_set_addr(ADDR, ADDR);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "%4d %s failed code:%d", __LINE__, __func__, err);
    return;
  }

  ESP_LOGI(TAG, "%4d %s success", __LINE__, __func__);

  return;
}

void test_ld100_get_state_and_relay() {
  esp_err_t err = ESP_OK;
  bool state = true;
  bool relay = true;

  err = modbus_ld100_get_state(ADDR, &state);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "%4d %s failed code:%d", __LINE__, __func__, err);
    return;
  }

  err = modbus_ld100_get_relay(ADDR, &relay);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "%4d %s failed code:%d", __LINE__, __func__, err);
    return;
  }

  ESP_LOGI(TAG, "%4d %s state:%d, relay:%d", __LINE__, __func__, state, relay);

  return;
}

void test_ld100_set_relay(bool state) {
  esp_err_t err = ESP_OK;

  err = modbus_ld100_set_relay(ADDR, state);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "%4d %s failed code:%d", __LINE__, __func__, err);
    return;
  }

  ESP_LOGI(TAG, "%4d %s to %d success", __LINE__, __func__, state);

  return;
}

// main func ==================================================================

void app_main() {
  esp_err_t err = ESP_OK;

  ESP_LOGI(TAG, "test begin");

  err = modbus_ld100_init(RS485_PORT, TX_PIN, RX_PIN, EN_PIN);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "%4d modbus_init failed", __LINE__);
    return;
  }

  // test get addr
  vTaskDelay(2000 / portTICK_RATE_MS);
  test_ld100_get_addr();

  // test set addr
  vTaskDelay(2000 / portTICK_RATE_MS);
  test_ld100_set_addr();

  // test set relay 1
  vTaskDelay(2000 / portTICK_RATE_MS);
  test_ld100_set_relay(true);

  // test set relay 0
  vTaskDelay(2000 / portTICK_RATE_MS);
  test_ld100_set_relay(false);

  ESP_LOGI(TAG, "test end");

  while (1) {
    test_ld100_get_state_and_relay();
    vTaskDelay(2000 / portTICK_RATE_MS);
  }
}
