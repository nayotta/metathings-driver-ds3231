...
static const char *TAG = "MODBUS_GZPD800T_HUAWEI_MAIN";
...
void app_main() {
  ...
  err = modbus_gzpd800T_init(RS485_PORT, TX_PIN, RX_PIN, EN_PIN,
                             GZPD800T_PORT_NUM);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "%4d %s modbus_gzpd800T_init failed", __LINE__, __func__);
    return;
  }
  ...
  module_flow->flow_handle = modbus_gzpd800T_get_flow_data;
